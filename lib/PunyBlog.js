...
module.exports = class PunyBlog {
      ...
      try {
        ...
        const options = {
          ...
          render_vars: {
            render: {
              current: {
                basename: this._get_file_basename(filename_md),
                path: this._get_file_path(filename_md),
              }
            }
          }
        };
  ...
  /**
   * Get the basename for a markdown file. Extension will be changed to .html.
   */
  _get_file_basename(filename) {
    return path.basename(filename.replace(this._config.path_src, ""))
      .replace(/\.md$/i, ".html");
  }

  /**
   * Get the relative path for a markdown file. (Relative to /src/ or /dest).
   * Path will never begin or end with a slash.
   */
  _get_file_path(filename) {
    return path.dirname(filename.replace(this._config.path_src, ""))
      .replace(/^\//, "")
      .replace(/\/$/, "");
  }
  ...
  /**
   * Return a json object with all the documents in the source path. Frontmatter for each
   * should be the value for each file.
   *
   * @return object - key/value pairs of filename and data
   *

...
module.exports = class RenderHtml {
  constructor(config = {}) {
    ...
    this._paths_partials = [ ...config.paths_partials, path.join(__dirname, "../partials") ];
    this.nunjucks_env = nunjucks.configure(this._paths_partials, options);
    this._config = config;
  }
...
`;
    if (this._paths_partials.length) {
      msg += "\nDefined partial paths:";
      this._paths_partials.forEach(my_path => {
        msg += `\n  - ${my_path}`;
      });
    }
